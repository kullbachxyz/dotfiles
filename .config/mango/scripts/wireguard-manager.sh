...
function askpass() {
  rofi -dmenu -password -no-fixed-num-lines -p "Sudo password : " -theme ~/.config/rofi/prompt-wal.rasi 
}
...
function toggle_wireguard() {
  status_wireguard && \
     SUDO_ASKPASS=~/.config/mango/scripts/wireguard-manager.sh sudo -A systemctl stop $SERVICE_NAME || \
     SUDO_ASKPASS=~/.config/mango/scripts/wireguard-manager.sh sudo -A systemctl start $SERVICE_NAME
}
